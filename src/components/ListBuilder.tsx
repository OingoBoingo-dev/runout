  ...
  return (
    <div>
      <div className="rounded-card bg-paper p-5 text-ink">
        <div className="grid gap-x-4 sm:grid-cols-2">
          ...
          <div className="mt-4">
            ...
            {entries.length > 0 && (
              <p className="mt-1.5 font-mono text-[11px] text-ink2">
            ...
            <div className="flex min-h-[44px] items-center gap-3">
              <span
                className={`rounded-full px-3 py-1 font-mono text-[10px] font-semibold uppercase tracking-[0.11em] ${
                  status === 'published'
                    ? 'bg-accent text-press'
                    : 'border border-dashed border-ink2 text-ink2'
                }`}
              >
                {status}
              </span>
              <span
                className={`font-mono text-xs tabular-nums ${atCap ? 'font-semibold text-[#A5341A]' : 'text-ink2'}`}
              >
        ...
        {error && <p className="mt-3 font-mono text-xs text-[#A5341A]">{error}</p>}
          ...
          {status === 'published' ? (
            <>
              ...
              <button
                ...
                disabled={pending}
                onClick={unpublish}
                className="rounded-chip border border-ink/20 px-5 py-3 font-mono text-xs uppercase tracking-wider hover:border-ink disabled:opacity-60"
              >
                Unpublish to draft
              </button>
            </>
      ...
      <div className="mt-5 grid items-start gap-5 lg:grid-cols-[340px_1fr]">
        <aside className="lg:sticky lg:top-[72px]">
          <div className="rounded-card bg-paper p-5 text-ink">
            ...
            <div className="mt-3 max-h-[52vh] overflow-y-auto">
              {searching && (
                <p className="py-3 font-mono text-xs text-ink2">
                  Searching… (queued at ~1 request/second)
                </p>
              )}
              {searchNote && !searching && (
                <p className="py-3 font-mono text-xs text-ink2">{searchNote}</p>
              )}
              {!searching &&
                results.map(r => {
                  const added = entries.some(e => e.mbid === r.mbid);
                  return (
                    <div key={r.mbid} className="flex items-center gap-2.5 border-b border-ink/10 py-2 last:border-0">
                      ...
                      <span className="min-w-0 flex-1">
                        ...
                        <span className="block truncate font-mono text-[11px] tabular-nums text-ink2">
                          {r.artist_name}
                          {fmtYear(r.year) ? ` · ${fmtYear(r.year)}` : ''}
                        </span>
                      </span>
                      {added ? (
                        <span className="flex-none rounded-chip border border-ink/20 px-2.5 py-1 font-mono text-[10px] uppercase text-ink2">
                          added
                        </span>
                      ) : (
                        <button
                          ...
                          disabled={atCap}
                          onClick={() => add(r)}
                          className="flex-none rounded-chip border border-ink/20 px-3 py-1.5 font-mono text-[11px] uppercase tracking-wide hover:border-accent hover:text-accent disabled:opacity-40"
                        >
                          Add
                        </button>
                      )}
              ...
              {!searching && !results.length && !searchNote && (
                <p className="py-3 font-mono text-[11px] leading-relaxed text-ink2">
        ...
        <section>
          <h2 className="mb-3 font-mono text-xs uppercase tracking-[0.16em] text-muted">
            Ranked entries — drag the grip or use the arrows
          </h2>
          <div className="rounded-card bg-paper py-1 text-ink">
            {entries.length === 0 && (
            ...
            {entries.map((e, i) => (
              <div
                ...
                className={`flex items-start gap-2.5 border-b border-ink/10 px-3 py-3 last:border-0 ${
                  overIdx === i ? 'shadow-[inset_0_3px_0_var(--color-accent)]' : ''
                }`}
              >
                <span
                  ...
                  className="mt-3 flex-none cursor-grab px-1.5 py-2 text-[#A39D92] active:cursor-grabbing"
                  aria-hidden
                >
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="block truncate font-mono text-xs tabular-nums text-ink2">
                    {e.artist}
                  ...
                  <textarea
                    ...
                    className="mt-2 min-h-[42px] w-full rounded-chip border border-ink/15 bg-white/50 p-2 text-[13px] focus:border-accent focus:outline-none"
                  />
                  <span className="mt-1 block text-right font-mono text-[10px] tabular-nums text-ink2">
                    {e.blurb.length}/280
                  </span>
                </span>
                <span className="flex flex-none flex-col gap-2">
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-chip border border-ink/20 font-mono text-[13px] hover:border-accent hover:text-accent disabled:opacity-40"
                  >
                    ↑
                  </button>
                  <button
                  ...
                  <button
                    ...
                    className="flex h-8 w-8 items-center justify-center rounded-chip border border-ink/20 font-mono text-[13px] hover:border-accent hover:text-accent"
                  >
                    ×
                  </button>
                </span>
